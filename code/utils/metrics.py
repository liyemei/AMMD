...
#        writer = pd.ExcelWriter('save/confusion_matrix.xlsx')
...
class ConfusionMatrix:
    ...
    def process_batch(self, detections, labels):
        ...
        for i, gc in enumerate(gt_classes):
            ...
            if n and sum(j) == 1:
                ...
                self.matrix[self.nc-1, gc] += 1  # background FP xiugai
    ...
    def plot(self, normalize=True, save_dir='', names=()):
        try:
            ...
            array = self.matrix 
            array= np.delete(array,6,axis = 0)
            array= np.delete(array,6,axis = 1)
            writer = pd.ExcelWriter('../results/save/confusion_matrix.xlsx')
            df= pd.DataFrame(array)
            print(save_dir)
            df.to_excel(writer, index=False,encoding='utf-8',sheet_name='Sheet1')
            writer.save()
